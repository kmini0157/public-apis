...
- **온디바이스 요약**: 각 묶음마다 LLM 없이 추출식 요약을 답니다. 묶음 센트로이드에
  ...
  **"AI 다듬기"를 켜면**(선택, 발췌문이 Puter.js로 전송됨) 그 불릿들을 LLM이 한 문장으로
  다듬어 ✨ 표시로 교체합니다 — 실패하면 추출식 요약이 그대로 남습니다.

### 🔁 중복/충돌 정리
다기기 동기화나 반복 수집으로 같은 글이 여러 번 들어올 수 있습니다. "🔁 중복/충돌 정리"의
*중복 찾기*를 누르면 **같은 출처**이거나 **임베딩이 거의 동일**한 노트를 묶어 보여줍니다.
**엄격도 슬라이더**(코사인 0.88–0.99, 기본 0.95)로 얼마나 비슷해야 중복으로 볼지 조절합니다.
각 그룹에서 *이거 남기고 정리*를 누르면 나머지를 삭제하며, 삭제는 tombstone으로
다른 기기에도 전파됩니다.

### 🕸 그래프 뷰
"🕸 그래프 뷰"에서 *그리기*를 누르면 노트가 점, 유사한 노트끼리 선으로 이어진
**유사도 네트워크**를 캔버스에 그립니다(포스 레이아웃, 결정적·의존성 0).
선의 진하기가 유사도이고, 임계값은 클러스터 민감도 슬라이더를 공유합니다.
...
└─ js/
   ...
   ├─ graph.js       # 유사도 네트워크 (그래프 빌드 + 포스 레이아웃)
...
## 로드맵
계획했던 기능은 모두 구현되었습니다 ✅ — 수집(URL·메모·파일·음성·웹클립·Radar), 로컬 임베딩
의미검색, 스트리밍 RAG 대화, 자동 태그·클러스터·요약(+AI 다듬기 옵션), 그래프 뷰,
중복 정리, PWA, 암호화 백업, 삭제까지 전파되는 E2E 다기기 동기화(+Realtime), tombstone GC.
이후 아이디어가 생기면 이슈로 남겨주세요.
